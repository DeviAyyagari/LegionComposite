...
//			if(hist.find(volume[i])!=hist.end()){
//				hist[volume[i]]++;
//			}
//			else{
//				hist[volume[i]] = 1;
...
//		cout << "Histogram:" << endl;
//		for(auto const &ent1 : hist) {
//		  // ent1.first is the first key
//		  cout << ent1.first << " : " << ent1.second << endl;
//		}
		cout << "Wrote " << i << " points to region" << endl; 
...
//			cout << "Discovered GPU: " << it->address_space() << endl;
			break;
		default:						// Something else...?
			break;
		}
	}
//	cout << "ALL_CPUS: " << all_cpus.size() << ", ALL_GPUS: " << all_gpus.size() << endl;
	{
...
//	else if(task->task_id == CREATE_INTERFACE_TASK_ID){
...
//		assert(zc_mem.exists());
//		
//		task->regions[0].target_ranking.push_back(sys_mem);
//		task->regions[0].virtual_map = false;
//		task->regions[0].enable_WAR_optimization = war_enabled;
//		task->regions[0].reduction_list = false;
//		// Make everything SOA
//		task->regions[0].blocking_factor = task->regions[0].max_blocking_factor;
//		
//
//		task->regions[1].target_ranking.push_back(zc_mem); 	//	and map them to the framebuffer memory
//		task->regions[1].virtual_map = false;
//		task->regions[1].enable_WAR_optimization = war_enabled;
//		task->regions[1].reduction_list = false;
//		// Make everything SOA
//		task->regions[1].blocking_factor = task->regions[1].max_blocking_factor;
//		
//
//		task->regions[2].target_ranking.push_back(sys_mem);
//		task->regions[2].virtual_map = false;
//		task->regions[2].enable_WAR_optimization = war_enabled;
//		task->regions[2].reduction_list = false;
//		// Make everything SOA
//		task->regions[2].blocking_factor = task->regions[2].max_blocking_factor;
//		
//	}
	else{
		// Put everything else in the system memory
