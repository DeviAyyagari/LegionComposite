...
int main(int argc, char **argv){
	...
	if (inFile.is_open()){
		...
		for(int i = 0; i < width*height*4;){
			...
			b = b>1.0 ? 1.0 : b;
			a = a>1.0 ? 1.0 : a;
			unsigned short rc = static_cast<unsigned short>(r*65535);
			unsigned short gc = static_cast<unsigned short>(g*65535);
			unsigned short bc = static_cast<unsigned short>(b*65535);
			unsigned short ac = static_cast<unsigned short>(a*65535);
			if(a<1.0 && (b > 0 || r > 0 || g > 0)){
			//	cout << bc <<", " << gc << ", " << rc << ", " << ac << endl;
				bs++;
			}
